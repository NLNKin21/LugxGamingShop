                        ...
                        <div class="filter-section">
                            ...
                            <div class="mb-4">
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" 
                                           name="category" value="BattleRoyale" id="catBR"
                                           <c:if test="${param.category != null && param.category.contains('BattleRoyale')}">checked</c:if>>
                                    <label class="form-check-label" for="catBR">Battle Royale</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" 
                                           name="category" value="MOBA" id="catMOBA"
                                           <c:if test="${param.category != null && param.category.contains('MOBA')}">checked</c:if>>
                                    <label class="form-check-label" for="catMOBA">MOBA</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" 
                                           name="category" value="FPS" id="catFPS"
                                           <c:if test="${param.category != null && param.category.contains('FPS')}">checked</c:if>>
                                    <label class="form-check-label" for="catFPS">FPS</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" 
                                           name="category" value="RPG" id="catRPG"
                                           <c:if test="${param.category != null && param.category.contains('RPG')}">checked</c:if>>
                                    <label class="form-check-label" for="catRPG">RPG</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" 
                                           name="category" value="Sports" id="catSports"
                                           <c:if test="${param.category != null && param.category.contains('Sports')}">checked</c:if>>
                                    <label class="form-check-label" for="catSports">Sports</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" 
                                           name="category" value="Adventure" id="catAdv"
                                           <c:if test="${param.category != null && param.category.contains('Adventure')}">checked</c:if>>
                                    <label class="form-check-label" for="catAdv">Adventure</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="checkbox" 
                                           name="category" value="Account" id="catAcc"
                                           <c:if test="${param.category != null && param.category.contains('Account')}">checked</c:if>>
                            ...
                            <div class="mb-4">
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" 
                                           name="maxPrice" value="0" id="priceAll"
                                           <c:if test="${param.maxPrice == null || param.maxPrice == '0'}">checked</c:if>>
                                    <label class="form-check-label" for="priceAll">Tất cả</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" 
                                           name="maxPrice" value="100000" id="price1"
                                           <c:if test="${param.maxPrice == '100000'}">checked</c:if>>
                                    <label class="form-check-label" for="price1">Dưới 100.000đ</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" 
                                           name="maxPrice" value="500000" id="price2"
                                           <c:if test="${param.maxPrice == '500000'}">checked</c:if>>
                                    <label class="form-check-label" for="price2">Dưới 500.000đ</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" 
                                           name="maxPrice" value="1000000" id="price3"
                                           <c:if test="${param.maxPrice == '1000000'}">checked</c:if>>
                                    <label class="form-check-label" for="price3">Dưới 1.000.000đ</label>
                                </div>
                                <div class="form-check mb-2">
                                    <input class="form-check-input" type="radio" 
                                           name="maxPrice" value="5000000" id="price4"
                                           <c:if test="${param.maxPrice == '5000000'}">checked</c:if>>
                            ...
                            <div class="mb-4">
                                <select name="sort" class="form-select sort-select">
                                    <option value="" <c:if test="${param.sort == null || param.sort == ''}">selected</c:if>>Mặc định</option>
                                    <option value="asc" <c:if test="${param.sort == 'asc'}">selected</c:if>>Giá thấp đến cao</option>
                                    <option value="desc" <c:if test="${param.sort == 'desc'}">selected</c:if>>Giá cao đến thấp</option>
                                </select>
                            </div>
                                        ...
                                        <div class="product-card">
                                            ...
                                            <div class="product-info">
                                                    ...
                                                    <c:choose>
                                                        <c:when test="${product.quantity > 0}">
                                                            <span class="text-success">
                                                                <i class="fa fa-check-circle"></i> Còn hàng (${product.quantity})
                                                            </span>
                                                        </c:when>
                                                        <c:otherwise>
                                                ...
                                                <div class="d-flex justify-content-between align-items-center">
                                                    <a href="/product/${product.id}" class="btn btn-add-cart btn-sm">
                                                        <i class="fa fa-eye"></i> Chi tiết
                                                    </a>
                                                    <c:if test="${product.quantity > 0}">
                                                        <form action="/add-product-to-cart/${product.id}" method="post" style="display: inline;">
                    ...
                    <c:if test="${totalPages > 1}">
                        <div class="pagination-custom">
                            ...
                            <c:if test="${currentPage > 1}">
                                <a href="/products?page=${currentPage - 1}&name=${param.name}&category=${param.category}&maxPrice=${param.maxPrice}&sort=${param.sort}">
                            ...
                            <c:forEach begin="1" end="${totalPages}" var="i">
                                <c:choose>
                                    ...
                                    <c:otherwise>
                                        <a href="/products?page=${i}&name=${param.name}&category=${param.category}&maxPrice=${param.maxPrice}&sort=${param.sort}">${i}</a>
                                    </c:otherwise>
                                </c:choose>
                            </c:forEach>
